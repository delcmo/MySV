...
VariableDepthRiverbed::VariableDepthRiverbed(const std::string & name, InputParameters parameters) :
...
Real
VariableDepthRiverbed::value(Real /*t*/, const Point & p)
{
  if ( p(0)<_xmin )
    return 0.;
  else if ( p(0)>_xmax )
...
RealVectorValue
VariableDepthRiverbed::gradient(Real /*t*/, const Point & p)
{
  if ( p(0)<_xmin )
    return 0.;
  else if ( p(0)>_xmax )
